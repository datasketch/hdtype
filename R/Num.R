...
new_Num <- function(x, skip_stats = FALSE){
  ...
  # %f: Floating point display. This can often be modified with precision, such as %.2f to display two digits after the decimal point.
  # %e or %E: Scientific notation. Lowercase 'e' or uppercase 'E' is used, respectively.
  # %g or %G: Uses the shorter of %e or %f. The 'E' or 'e' in the output depends on the specifier used.
  # %x or %X: Unsigned hexadecimal integer (lowercase 'x' for lowercase letters, uppercase 'X' for uppercase letters).
  ...
  vctrs::new_vctr(x, format = list(spec = "%f"),
                  stats = stats, class = "hd_Num")
}
